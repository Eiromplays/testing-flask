...
original_author="Eiromplays"
original_name="testing_flask"
original_urlname="testing-flask"
original_description="Awesome testing_flask created by Eiromplays" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/testing_flask "${name}"
...
then
    ...
    echo "then run 'testing_flask' or 'python -m testing_flask'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'testing_flask' or 'python -m testing_flask' or 'poetry run testing_flask'"
fi
